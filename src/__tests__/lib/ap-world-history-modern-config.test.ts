...
import { EXAM_CONFIG } from "@/lib/ap-world-history-modern/config";

describe("AP World History: Modern Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM182");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("ap-world-history-modern");
  });
